...
if ($id <= 0) {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
      ...
      $ok = "Mutation with ID {$id} has been updated.";
    } catch (Throwable $e) {
  ...
  $mut = $mutation->find($id);
  if (!$mut) {
    $err = "Mutation with ID {$id} not found.";
  }
}
?>
<!doctype html>
...
<body>
  <header class="hero">
    <div class="wrap">
      <div>
        ...
        <nav class="links">
          <a href="patient_search.php">Search Mutations</a>
          <a href="patient_create.php">Create Mutation</a>
        </nav>
      </div>
    </div>
  </header>
  ...
  <div class="container">
    ...
    <?php if (!empty($patient)): ?>
      <section class="card">
        <h2>Edit Mutation #<?= (int)$patient['patient_id'] ?></h2>
        <form method="post">
          <div class="field">
